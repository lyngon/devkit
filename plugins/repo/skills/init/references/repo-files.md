...
- This repository follows the Lyngon structure. Packages are laid out by kind: `apps/` entrypoints, `libs/` libraries, `contracts/` schemas with generated clients, `tools/` repository-internal executables, `infra/` modules and environments. Create one with `/repo:add-package`, never by hand. {Structure scope only; keep the first sentence verbatim, the session hook and `add-package` look for it.}
{One line per existing package: path and what it is.}
- `docs/adr/NNNN-slug.md`: decisions about the repository. A context's decisions live in its core library, `libs/<ctx>/docs/adr/`.
...
- `CLAUDE.md` is the source file; `AGENTS.md` is a symlink to it. Edit this file. Every package has its own pair holding only what differs from this one.
- One workspace and one lockfile per language at the root; members are listed explicitly. Internal libraries carry no version; a breaking change updates every consumer in the same commit.
...
Packages are laid out by kind; see the Lyngon devkit's `STRUCTURE.md` for the rules.
...
Every package named in the interview is created with the `add-package` skill of this plugin, one invocation per package, core libraries first.
It decides the directory from the kind, creates the manifest with the stack's own initializer, writes `devenv.nix`, `README.md`, `INTENT.md`, `CLAUDE.md` and the `AGENTS.md` symlink, and registers the package in its workspace, the root `devenv.yaml` and the root `CLAUDE.md`.
The root workspace files are written before the first package of each language; their templates are in `add-package`'s `workspaces.md` reference.
